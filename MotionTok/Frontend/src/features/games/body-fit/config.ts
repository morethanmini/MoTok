/**
 * 게임④ 그대로 멈춰라 — 튜닝 값 단일 설정 객체 (UI 스펙 §10).
 *
...
export function defaultConfig(): BodyFitConfig {
  return {
    filter: {
      ...
      // 그대로 쓰이면서 떨렸다(2026-07-28 랩 계측). 그 관절만 강하게 누른다.
      // ⚠ 이 값 자체는 아직 실기 미검증 — 위 minCutoff 0.6만 눈으로 확인된 값이다.
      // /dev/avatar-lab의 "신뢰도 감쇠" 슬라이더로 0↔1을 오가며 확정할 것.
      visTighten: 0.6,
    },
    avatar: {
