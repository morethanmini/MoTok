...
/**
 * 아바타·포디움을 월드에 놓을 때의 배율 (2026-07-28).
 *
 * 아바타가 벽을 거의 꽉 채워 무대가 답답했다. 벽(wall.ts)은 그대로 두고 아바타와
 * 포디움만 줄여 "넓은 벽 + 작은 인형" 구도를 만든다. 카메라·안개·벽 접근 애니메이션은
 * 손대지 않는다 — 그래서 이 값 하나만 바꾸면 구도 전체가 따라온다.
 *
 ...
 * 축소 후 아바타가 화면 정중앙에 떠 보여서 하단으로 앉힌다.
 * 벽 구멍도 같은 만큼 내려야 정렬이 유지된다(wall.ts inViewport).
 */
