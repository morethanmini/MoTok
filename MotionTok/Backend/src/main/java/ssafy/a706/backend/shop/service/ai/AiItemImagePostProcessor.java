 ...
 * fal의 배경제거 엔드포인트를 한 번 더 부르면 호출·대기·실패 지점이 하나씩 늘어난다.
 ...
 *
 * <p><b>돈 때문은 아니다.</b> 한때 "배경제거를 붙이면 장당 크레딧이 두 배"라고 적어 뒀는데
 * 사실이 아니었다 — 생성이 $0.035/MP인데 fal의 rembg 엔드포인트(smoretalk-ai/rembg-enhance)는
 * 장당 $0.006, birefnet/v2는 컴퓨트 초당 $0.0008로 두 배가 아니라 +3~17% 수준이다. 그러니
 * 임계값 {@code app.shop.fal.background-threshold}를 조절해도 배경이 깔끔히 떨어지지 않으면
 * 값 비싸서 못 붙이는 게 아니라 <b>붙일 만한</b> 선택지로 두고 보면 된다(워커가 쓰는 rembg와
 * 같은 모델이다).</p>
 */
@Slf4j
@Component
