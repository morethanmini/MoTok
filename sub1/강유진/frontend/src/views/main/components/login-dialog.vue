    ...
    <div class="login-dialog">
      ...
      <form @submit.prevent="clickLogin" ref="loginForm">
        ...
        <div class="dialog-footer">
          <button type="submit" class="btn-primary" :disabled="state.isSubmitting">로그인</button>
        </div>
      </form>
  ...
  setup(props, { emit }) {
    ...
    const clickLogin = async () => {
      ...
      try {
        ...
        // SPEC.md 3.1 - 계정 없음(404) '존재하지 않는 계정입니다.' / 비밀번호 불일치(401) '잘못된 비밀번호입니다.'
        const message = error.response && error.response.data && error.response.data.message
        ElMessage.error(message || '로그인에 실패했습니다.')
      } finally {
