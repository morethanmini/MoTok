...
public class UserController {
	...
	@PostMapping()
	@ApiOperation(value = "회원 가입", notes = "소속/직책/이름/아이디/패스워드를 통해 회원가입 한다.")
    @ApiResponses({
	...
	@GetMapping("/{userId}")
	@ApiOperation(value = "유저 정보 조회 (존재하는 회원 확인용)", notes = "회원가입 시 아이디 중복 여부를 확인한다.")
	@ApiResponses({
	...
	@PatchMapping("/{userId}")
	@ApiOperation(value = "유저 정보 수정", notes = "로그인한 본인의 소속/직책/이름 정보를 수정한다.")
	@ApiResponses({
		@ApiResponse(code = 200, message = "성공"),
		@ApiResponse(code = 401, message = "인증 실패"),
		@ApiResponse(code = 403, message = "권한 없음")
	...
	public ResponseEntity<? extends BaseResponseBody> updateUserInfo(
			...
			@RequestBody @ApiParam(value = "수정할 유저 정보", required = true) UserPatchReq patchInfo,
			@ApiIgnore Authentication authentication) {
		SsafyUserDetails userDetails = (SsafyUserDetails) authentication.getDetails();
		if (!userDetails.getUsername().equals(userId)) {
			return ResponseEntity.status(403).body(BaseResponseBody.of(403, "본인 정보만 수정할 수 있습니다."));
		}
		userService.updateUser(userId, patchInfo);
		return ResponseEntity.status(200).body(BaseResponseBody.of(200, "Success"));
	}

	@DeleteMapping("/{userId}")
	@ApiOperation(value = "유저 정보 삭제 (탈퇴)", notes = "로그인한 본인의 계정 및 생성한 방, 회의 이력을 모두 삭제한다.")
	@ApiResponses({
	...
	public ResponseEntity<? extends BaseResponseBody> deleteUserInfo(
		...
		SsafyUserDetails userDetails = (SsafyUserDetails) authentication.getDetails();
		if (!userDetails.getUsername().equals(userId)) {
			return ResponseEntity.status(403).body(BaseResponseBody.of(403, "본인 정보만 삭제할 수 있습니다."));
		}
