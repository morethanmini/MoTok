...
import { createLoop, DEFAULT_LOOP, seededRng } from '../loop'
...
describe('루프 — 캐스팅', () => {
  it('IDLE에서 던지면 CASTING으로 가고 찌가 보인다', () => {
    ...
    expect(loop.cast(400, 0.5)).toBe(true)
  ...
  it('CASTING 중에는 다시 던질 수 없다 — 연타로 상태가 깨지지 않는다', () => {
    const loop = createLoop()
    loop.cast(400, 0.5)
    expect(loop.cast(200, 0.5)).toBe(false)
  })

  it('비행이 끝나면 조준 x에 착수하고 WAITING으로 간다', () => {
    const loop = createLoop()
    loop.cast(400, 0.5)
    run(loop, DEFAULT_LOOP.castFlightSec + 0.1)
    const s = loop.state()
    expect(s.phase).toBe('waiting')
    expect(s.bobber.x).toBeCloseTo(400, 0)
  })

  it('세게 던지면(power 1) 수평선 쪽에, 약하게 던지면 발밑에 떨어진다', () => {
    ...
    // 2D 바다에서는 위쪽이 먼 바다다(y가 작다)
    expect(far.state().bobber.y).toBeLessThan(near.state().bobber.y)
    expect(near.state().bobber.y).toBe(DEFAULT_LOOP.height - DEFAULT_LOOP.landNearMarginPx)
    expect(far.state().bobber.y).toBe(DEFAULT_LOOP.waterY + DEFAULT_LOOP.landFarMarginPx)
  })

  it('조준 x가 화면 밖이면 안쪽으로 잘린다', () => {
    const loop = createLoop()
    loop.cast(-500, 0.5)
    run(loop, DEFAULT_LOOP.castFlightSec + 0.1)
    expect(loop.state().bobber.x).toBeGreaterThanOrEqual(0)
    expect(loop.state().bobber.x).toBeLessThanOrEqual(DEFAULT_LOOP.width)
  })
})
...
describe('루프 — 힘겨루기와 결과', () => {
  ...
  it('RESULT가 지나면 IDLE로 돌아가 다시 던질 수 있다', () => {
    ...
    expect(loop.cast(300, 0.5)).toBe(true)
  })
