    ...
    <div class="hide-on-small">
      <div class="tool-wrapper">
        ...
        <div class="button-wrapper">
          ...
          <button v-else @click="clickLogout">로그아웃</button>
        </div>
      </div>
    </div>
      ...
      <div class="mobile-sidebar-wrapper" v-if="!state.isCollapse">
        <div class="mobile-sidebar">
          <div class="mobile-sidebar-tool-wrapper">
            ...
            <button v-else class="mobile-sidebar-btn login-btn" @click="clickLogout">로그아웃</button>
          </div>
          <ul class="menu">
            <li
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const state = reactive({
      ...
      isLogin: computed(() => store.getters['accountStore/isLogin'])
    })

    const menuSelect = function (item) {
      // 로그아웃 메뉴: 인증토큰 관리 기능에 따라 로그아웃 처리
      if (item.key === 'logout') {
